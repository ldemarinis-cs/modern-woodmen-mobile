...
// Props needed: title, rightAction, scrolled, onLogoPress
...
export function NavBar({ title, rightAction, scrolled = false, onLogoPress }: NavBarProps) {
  const isSubPage = !!title

  return (
    <div className="bg-brand flex items-center justify-between px-4 h-[56px] shrink-0 overflow-hidden">

      {/* Left — shield mark (always visible on sub-pages, animated on home) */}
      ...
      <div className="w-[44px] flex items-center justify-end">
        {rightAction ?? (
          <button className="flex items-center justify-center w-[44px] h-[44px] text-white active:opacity-70 transition-opacity relative">
